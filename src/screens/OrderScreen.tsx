...
export default function OrderScreen({ table, onBack }: Props) {
  ...
  const printByClient = async () => {
    ...
    for (let gi = 0; gi < slots.length; gi++) {
      ...
      const clientOriginalSubtotal = clientItems.reduce((a: number, i: any) => {
        const op = i.product?.price || i.unit_price;
        const ma = (i.item_modifiers || []).reduce((s: number, m: any) => s + (m.price_adjust || 0), 0);
        return a + (op + ma) * i.quantity;
      }, 0);
  ...
  const sendCartToKitchen = async () => {
    ...
    try {
      const items = cart.map(c => {
        ...
        const repeatGroup = groups.find(g => g.type === 'multi' && g.max_select > 1 && g.required);
  ...
  // Subtotal original (sin descuentos de producto) para mostrar en precuenta y calcular propina
  const unpaidOriginalSubtotal = unpaidItems.reduce((a, i) => {
    const origPrice = i.product?.price || i.unit_price;
    const modAdjust = (i.item_modifiers || []).reduce((s: number, m: any) => s + (m.price_adjust || 0), 0);
    return a + (origPrice + modAdjust) * i.quantity;
  }, 0);
  const unpaidProductDiscount = unpaidOriginalSubtotal - unpaidSubtotal;
  // Excluir items promo del descuento adicional de orden
    ...
    <View style={s.c}>
      ...
      <Modal visible={preCuentaModal} transparent animationType="fade">
        <View style={s.ov}><ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center', padding: 16 }}><View style={[s.md, { maxWidth: 520, width: width * 0.95 }]}>
          ...
          {discountAmount > 0 && <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4, backgroundColor: '#f0fdf4', borderRadius: 6, paddingHorizontal: 8, marginVertical: 4 }}><Text style={{ fontSize: 14, fontWeight: '700', color: COLORS.success }}>Descuento adicional {discountType === 'percent' ? `(${discountValue}%)` : ''}</Text><Text style={{ fontSize: 14, fontWeight: '700', color: COLORS.success }}>-{fmt(discountAmount)}</Text></View>}
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4, borderTopWidth: 1, borderTopColor: COLORS.border, marginTop: 4, paddingTop: 8 }}><Text style={{ fontSize: 13, color: COLORS.textSecondary }}>Propina sugerida 10%</Text><Text style={{ fontSize: 13, color: COLORS.warning }}>{fmt(Math.round(unpaidOriginalSubtotal * 0.1))}</Text></View>
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 20, flexWrap: 'wrap' }}>
            ...
            <TouchableOpacity disabled={isPrinting} style={[s.bOk, { backgroundColor: COLORS.warning, opacity: isPrinting ? 0.5 : 1 }]} onPress={async () => { if (isPrinting) return; setIsPrinting(true); try { if (order) await supabase.from('orders').update({ discount_type: discountType, discount_value: parseInt(discountValue) || 0, subtotal: unpaidSubtotal, total: unpaidTotal }).eq('id', order.id); await supabase.from('tables').update({ status: 'cuenta' }).eq('id', table.id); playClickPOS(); setIsPrinting(false); setPreCuentaModal(false); onBack(); } catch (e: any) { setIsPrinting(false); if (typeof window !== 'undefined') window.alert('Error: ' + e.message); } }}><Text style={s.bOkT}>{isPrinting ? '⏳ Imprimiendo...' : `🖨 ${payMode === 'partial' && selectedItems.length > 0 ? `Imprimir (${selectedItems.length})` : 'Imprimir'}`}</Text></TouchableOpacity>
      ...
      <Modal visible={paySelectedModal} transparent animationType="fade">
        <View style={s.ov}><ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center', padding: 16 }}><View style={[s.md, { maxWidth: 480, width: width * 0.92 }]}>
          ...
          <View style={s.payG}>{['efectivo', 'tarjeta', 'transferencia', 'pedidosya', 'consumo', 'cortesia_cumple', 'cortesia_dj'].map(m => <TouchableOpacity key={m} style={[s.payO, paymentMethod === m && s.payOA]} onPress={() => setPaymentMethod(m)}><Text style={{ fontSize: 18 }}>{m === 'efectivo' ? '💵' : m === 'tarjeta' ? '💳' : m === 'transferencia' ? '📱' : m === 'pedidosya' ? '🛵' : m === 'consumo' ? '🍽️' : m === 'cortesia_cumple' ? '🎂' : '🎵'}</Text><Text style={[s.payL, paymentMethod === m && { color: COLORS.primary }]}>{m === 'efectivo' ? 'Efectivo' : m === 'tarjeta' ? 'Tarjeta' : m === 'transferencia' ? 'Transferencia' : m === 'pedidosya' ? 'PedidosYa' : m === 'consumo' ? 'Consumo' : m === 'cortesia_cumple' ? 'Cortesía Cumple' : 'Cortesía DJ'}</Text></TouchableOpacity>)}</View>
        ...
        <View style={s.ov}><ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center', padding: 16 }}><View style={[s.md, { maxWidth: 520, width: width * 0.95 }]}>
            ...
            {tipEntries.map((te, i) => (
              <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 6 }}>
                <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.background, borderRadius: 8, borderWidth: 1, borderColor: COLORS.border, overflow: 'hidden' }}>
                  <TouchableOpacity onPress={() => { const methods = ['efectivo','tarjeta','transferencia','pedidosya','consumo','cortesia_cumple','cortesia_dj']; const next = methods[(methods.indexOf(te.method) + 1) % methods.length]; updateTipEntry(i, 'method', next); }} style={{ paddingHorizontal: 10, paddingVertical: 10, backgroundColor: COLORS.card, borderRightWidth: 1, borderRightColor: COLORS.border }}>
                    <Text style={{ fontSize: 12, color: COLORS.text, minWidth: 80 }}>{te.method === 'efectivo' ? '💵 Efectivo' : te.method === 'tarjeta' ? '💳 Tarjeta' : te.method === 'transferencia' ? '📱 Transf.' : te.method === 'pedidosya' ? '🛵 PedidosYa' : te.method === 'consumo' ? '🍽️ Consumo' : te.method === 'cortesia_cumple' ? '🎂 Cortesía' : '🎵 DJ'}</Text>
                  </TouchableOpacity>
          ...
          <View style={{ marginTop: 16 }}>
            ...
            {payEntries.map((pe, i) => (
              <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 6 }}>
                <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.background, borderRadius: 8, borderWidth: 1, borderColor: COLORS.border, overflow: 'hidden' }}>
                  <TouchableOpacity onPress={() => { const methods = ['efectivo','tarjeta','transferencia','pedidosya','consumo','cortesia_cumple','cortesia_dj']; const next = methods[(methods.indexOf(pe.method) + 1) % methods.length]; updatePayEntry(i, 'method', next); }} style={{ paddingHorizontal: 10, paddingVertical: 10, backgroundColor: COLORS.card, borderRightWidth: 1, borderRightColor: COLORS.border }}>
                    <Text style={{ fontSize: 12, color: COLORS.text, minWidth: 80 }}>{pe.method === 'efectivo' ? '💵 Efectivo' : pe.method === 'tarjeta' ? '💳 Tarjeta' : pe.method === 'transferencia' ? '📱 Transf.' : pe.method === 'pedidosya' ? '🛵 PedidosYa' : pe.method === 'consumo' ? '🍽️ Consumo' : pe.method === 'cortesia_cumple' ? '🎂 Cortesía' : '🎵 DJ'}</Text>
                  </TouchableOpacity>
                  <Text style={{ paddingHorizontal: 8, fontSize: 14, color: COLORS.textSecondary }}>$</Text>
