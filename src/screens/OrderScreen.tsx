    ...
    <View style={s.c}>
      ...
      <Modal visible={paySelectedModal} transparent animationType="fade">
        <View style={s.ov}><ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center', padding: 16 }}><View style={[s.md, { maxWidth: 480, width: width * 0.92 }]}>
          ...
          <View style={s.payG}>{['efectivo', 'tarjeta', 'transferencia', 'pedidosya', 'consumo'].map(m => <TouchableOpacity key={m} style={[s.payO, paymentMethod === m && s.payOA]} onPress={() => setPaymentMethod(m)}><Text style={{ fontSize: 18 }}>{m === 'efectivo' ? '💵' : m === 'tarjeta' ? '💳' : m === 'transferencia' ? '📱' : m === 'pedidosya' ? '🛵' : '🍽️'}</Text><Text style={[s.payL, paymentMethod === m && { color: COLORS.primary }]}>{m === 'efectivo' ? 'Efectivo' : m === 'tarjeta' ? 'Tarjeta' : m === 'transferencia' ? 'Transferencia' : m === 'pedidosya' ? 'PedidosYa' : 'Consumo'}</Text></TouchableOpacity>)}</View>
