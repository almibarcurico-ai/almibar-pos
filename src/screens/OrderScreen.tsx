  ...
  const printByClient = async () => {
    ...
    for (let gi = 0; gi < slots.length; gi++) {
      ...
      const ticket = generateBoleta({
        table: table.number,
        waiter: waiterName || '',
        items: clientItems.map((i: any) => ({ name: i.product?.name || '', qty: i.quantity, price: i.unit_price, total: i.total_price })),
      ...
      <Modal visible={preCuentaModal} transparent animationType="fade">
        <View style={s.ov}><ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center', padding: 16 }}><View style={[s.md, { maxWidth: 520, width: width * 0.95 }]}>
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 20, flexWrap: 'wrap' }}>
            ...
            <TouchableOpacity style={[s.bOk, { backgroundColor: COLORS.warning }]} onPress={async () => { try { if (order) await supabase.from('orders').update({ discount_type: discountType, discount_value: discountAmount, subtotal: unpaidSubtotal, total: unpaidTotal }).eq('id', order.id); await supabase.from('tables').update({ status: 'cuenta' }).eq('id', table.id); const cajaIp = PRINTER_CONFIG.caja?.ip || PRINTER_CONFIG.barra?.ip; const cajaPort = PRINTER_CONFIG.caja?.port || PRINTER_CONFIG.barra?.port || 9100; if (cajaIp) { const ticket = generateBoleta({ table: table.number, waiter: waiterName || '', items: unpaidItems.map((i: any) => ({ name: i.product?.name || '', qty: i.quantity, price: i.unit_price, total: i.total_price })), subtotal: unpaidSubtotal, discount: discountAmount, discountLabel: discountType === 'percent' ? `Dcto (${discountValue}%)` : undefined, tip: 0, total: unpaidTotal, payments: [], orderNumber: order?.order_number }); await sendToPrinter(cajaIp, cajaPort, ticket, 'caja'); } playClickPOS(); setPreCuentaModal(false); onBack(); } catch (e: any) { if (typeof window !== 'undefined') window.alert('Error: ' + e.message); } }}><Text style={s.bOkT}>🖨 Imprimir</Text></TouchableOpacity>
